...
route.get('/notice', async (req, res) => {
  try {
    ...
    let { acc } = req.query,
      carr = [];
    if (acc === 'chang') {
      carr = await queryData('chat', '*', `WHERE flag=? AND state=?`, ['chang', '0'])
    } else {
      carr = await queryData('chat', '*', `WHERE flag in(?,?) AND state = ?`, [`${account}-${acc}`, `${acc}-${account}`, '0']);
    }
    carr = carr.map(item => {
      delete item.state
      return item;
...
route.get('/getmsg', async (req, res) => {
  try {
    ...
    let tt = Date.now();
    // 标记已读
    let r = await updateData('friends', { islooK: 'y', time: tt }, `WHERE friend=? AND account=?`, [acc, account]);
    if (r.changes == 0) {
      if (acc === 'chang') {
        ...
        let user = await queryData('user', 'account', `WHERE state = ? AND account = ?`, ['0', acc]);
        if (user.length > 0) {
          await insertData('friends',
            [
              {
...
route.post("/delmsg", async (req, res) => {
  try {
    ...
    await updateData('chat', { state: '1' }, `WHERE id=? AND state=? AND _from=?`, [tt, '0', account])
...
route.post("/clearmsg", async (req, res) => {
  try {
    ...
    if (ac === 'chang') {
      if (account === 'root') {
        await updateData('chat', { state: '1' }, `WHERE _to=? AND state=?`, ['chang', '0'])
        _success(res)
      } else {
        _err(res)
      }
    } else {
      await updateData('chat', { state: '1' }, `WHERE flag in(?,?) AND state = ?`, [`${account}-${ac}`, `${ac}-${account}`, '0']);
      _success(res);
    }
