...
route.get('/getlist', async (req, res) => {
  try {
    ...
    let home = bookSort(await queryData('bookmk', 'id,name,link,logo,num', `WHERE listid=? AND state=? AND account=?`, ['home', '0', account]));
...
route.post('/move', async (req, res) => {
  try {
    ...
    let arr = bookSort(await queryData('bookmk', 'id,num', `WHERE listid=? AND state=? AND account=?`, ['home', '0', account]))
...
route.post('/bmktolist', async (req, res) => {
  try {
    ...
    let total = (await queryData('bookmk', "MAX(num)", `WHERE listid=? AND account=? AND state=?`, [nid, account, '0']))[0]['MAX(num)'] || 0
    let ob = [
      {
        key: 'num',
        where: 'id',
    ...
    await batchUpdateData('bookmk', ob, `account="${account}" AND state="0" AND listid="home"`)
    _success(res)
  } catch (error) {
    await writelog(req, `[${req._pathUrl}] ${error}`)
    _err(res)
...
route.post('/setlogo', async (req, res) => {
  try {
    ...
    await updateData('bookmk', { logo: purl }, `WHERE account=? AND state=? AND id=? AND listid=?`, [account, '0', id, 'home'])
...
route.post('/editbmk', async (req, res) => {
  try {
    ...
    await updateData('bookmk', { name, logo, link }, `WHERE account=? AND state=? AND id=? AND listid=?`, [account, '0', id, 'home'])
...
route.post('/delbmk', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let { arr } = req.body;
    await updateData('bookmk', { state: '1' }, `WHERE id IN (${new Array(arr.length).fill('?').join(',')}) AND state=? AND account=? AND listid=?`, [...arr, '0', account, 'home'])
...
route.post('/addbmk', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let obj = req.body;
    let total = (await queryData('bookmk', "MAX(num)", `WHERE listid=? AND account=? AND state=?`, ['home', account, '0']))[0]['MAX(num)'] || 0
