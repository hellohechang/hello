...
~function () {
  ...
  box.addEventListener('click', function (e) {
    ...
    callback && callback(e, rightBox.querySelectorAll('input'))
  })
  box.addEventListener('keyup', function (e) {
    if (e.target.tagName === 'INPUT' && e.key === 'Enter') {
      box.querySelector('.mtcbtn').click()
    }
  })
  function close() {
    box.style.opacity = 0
  ...
  function rikey(e) {
    ...
    if (!e || (ww <= 800 && rightBox.querySelectorAll('input').length > 0)) {
      rightBox.style.top = '50%'
      rightBox.style.left = '50%'
      rightBox.style.transform = 'translate(-50%,-50%)'
      return
    }
    let h = window.innerHeight,
      mtw = rightBox.offsetWidth,
      mth = rightBox.offsetHeight,
      x = e.clientX,
      y = e.clientY;
    x < ww / 2 ? null : (x = x - mtw);
    y < h / 2 ? null : (y = y - mth);
    ...
    rightBox.style.top = y + 'px'
    rightBox.style.left = x + 'px'
    rightBox.style.transform = 'none'
  }
  window.rightMenu = { open, close }
}();
// 本地储存
...
function playSound(src) {
  ...
  sound.src = src
  let v = _getData('lastvol') === null ? 0.7 : _getData('lastvol');
  sound.volume = v
  sound.play()
...
~function () {
  ...
  document.body.appendChild(box)
  copy.addEventListener('click', debounce(function () {
    copyText(text)
  }, 1000, true))
  edit.addEventListener('click', debounce(function () {
...
// 判断网址
function isurl(url) {
  let reg = /^(?:(http|https|ftp):\/\/)((?:[\w-]+\.)+[a-z0-9]+)(:[0-9]+)?((?:\/[^/?#]*)+)?(\?[^#]+)?(#.+)?$/i;
  return reg.test(url);
}
// 获取url域名
...
function _getAjax(url, data, y, time) {
  if (!y) {
    _loadingBar.start()
  }
  url = `${serverURL}${url}`;
...
// 上传文件
function _upFile(url, file, callback) {
  url = `${serverURL}${url}`
  return new Promise((resolve, reject) => {
...
function _each(obj, callback, context) {
  ...
  Object.prototype.toString.call(callback) === '[object Function]' ? null : callback = Function.prototype;
...
window.alert = function () {
  ...
  return function anonymous(content = '', options = {}) {
    ...
    if (options.hasOwnProperty('button') && Object.prototype.toString.call(options.button) === '[object Object]') {
...
~function () {
  ...
  function _err(str, again) {
    playSound(`${sideURL}/img/error.mp3`)
    mstc(false, str, again)
  }
  window._success = _success
  window._err = _err
}()
...
// 黑白
let _pagecolor = _getData('pagecolor');
_pagecolor === null ? _pagecolor = 0 : null;
document.documentElement.style.filter = `grayscale(${_pagecolor})`
// ~function ($) {
//   $.fn.snow = function (options) {
//     var $flake = $('<div id="snowbox" />').css({ 'position': 'fixed', 'pointer-events': 'none', 'z-index': '999', 'top': '-50px' }).html('&#10052;'),
...
~function () {
  ...
  function startSakura() {
    ...
    canvas.setAttribute('style', `position: fixed;left: 0;top: 0;pointer-events: none;z-index: ${LevelObj.hua};`);
    ...
    for (let i = 0; i < sakuraNum; i++) {
      ...
      sakura = new Sakura(randomX, randomY, randomS, randomR, {//生成一片樱花实例
        x: randomFnx,
        y: randomFny,
        r: randomFnR
      }, i);
...
function _position(el, relativeToHTML) {
...
// 事件委派获取点击目标
function _getTarget(e, targetStr, current) {
  let target = e.target,
...
function _progressBar(percent, callback) {
  let box = document.createElement('div')
  box.style.cssText = `
    position: fixed;
    top: 0;
  ...
  function calculationPosition(per) {
    ...
    let val = (pro1Box.offsetWidth - dolt.offsetWidth) * per + dolt.offsetWidth / 2;
    pro2Box.style.width = val + 'px';
    percentBox.innerText = parseInt(per * 100) + '%'
  }
  function move(e) {
    percent = (e.clientX - pro1BoxL - dolt.offsetWidth / 2) / (pro1Box.offsetWidth - dolt.offsetWidth);
...
~function () {
  ...
  function handleFontType() {
    ...
    let fontUrl = fontType === 'consolas.ttf' ? '/css/consolas.ttf' : `/getfile/font/${fontType}`
...
function fileLogoType(fname) {
  if (/(\.JPG|\.PNG|\.GIF|\.JPEG)$/gi.test(fname)) {
    ...
    return "icon-ppt";
  } else if (/(\.rar|\.7z|\.zip|\.ar|\.bz|\.car|\.dar|\.cpgz|\.f|\.ha)$/gi.test(fname)) {
    return "icon-filezip";
