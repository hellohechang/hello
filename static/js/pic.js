~function () {
  ...
  function hdUpFile(files){
    ~async function fn(num){
      ...
      try {
        ...
        let isrepeat = await _postAjax('/pic/repeatfile', { name: `${HASH}.${suffix}` }) //是否已经存在文件
  ...
  $allbg.on("click", ".bgitem", function () {
    let psrc = $(this).children(".bgitemimg").attr("data-src"),
      pp = psrc.replace('picys', 'pic');
    imgPreview(pp, psrc)
    ...
    bglazyImg($allbg, '.bgitem', '.bgitemimg');
  }, 100)).on('click', '.pagingbox', debounce(function (e) {
    let target = e.target,
