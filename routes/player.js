...
route.get('/musicshare', async (req, res) => {
  try {
    let id = req.query.id;
    let arr = await queryData('share', '*', `WHERE id=? AND type=?`, [id, 'music']);
    if (arr.length === 0) {
      _err(res, '分享已被取消~')
      return
    }
...
route.get('/search', async (req, res) => {
  try {
    let { a } = req.query,
      arr = JSON.parse((await queryData('musicinfo', 'data', `WHERE account=?`, ['root']))[0].data),
      ar = [];
    if (a) {
      arr.forEach((v, i) => {
        if (i < 2) return;
...
route.get('/getall', async (req, res) => {
  try {
    let arr = JSON.parse((await queryData('musicinfo', 'data', `WHERE account=?`, ['root']))[0].data),
      ar = [];
    arr.forEach((v, i) => {
      if (i < 2) return;
      v.item.forEach(y => {
...
route.get('/getlist', async (req, res) => {
  try {
    ...
    let { id } = req.query;
    let arr = await queryData('musicinfo', '*', `WHERE account IN (?,?)`, ["root", account]);
    let rarr = JSON.parse(arr.filter(item => item.account === 'root')[0].data)
    if (account !== 'root') {
      let aarr = JSON.parse(arr.filter(item => item.account === account)[0].data)
...
route.get('/getmusicinfo', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let obj = await queryData('lastmusic', 'data', `WHERE account=?`, [account]);
    if (obj.length !== 0) {
...
route.post('/editlist', async (req, res) => {
  try {
    ...
    let i = arr.findIndex(item => item.id === id);
    if (i > 1) {
      index < 2 ? index = 2 : (index > arr.length - 1 ? index = arr.length - 1 : null);
      arr[i].name = name;
