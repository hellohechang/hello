...
function getShareData() {
  _getAjax('/file/share', { id: HASH, pass: passCode })
    .then((res) => {
      if (mClose) {
        mClose && mClose(1);
        mClose = null;
      ...
      if (res.code == 0) {
        let { username, logo, account, data, valid, title } = res.data;
        uObj = { username, account };
        shareObj = data;
        logo = logo
...
async function openDir(path, top) {
  try {
    ...
    curmb.setPath(path);
    const res = await _getAjax('/file/readsharedir', {
      path,
      flag: `${HASH}/${passCode}`,
    });
    if (res.code == 0) {
...
async function readFileAndDir(obj) {
  ...
  if (type == 'dir') {
    ...
    try {
      const res = await _getAjax(
        '/file/getsharefile',
        { path: p, flag: `${HASH}/${passCode}` },
        { timeout: 0 }
      );
      if (res.code == 0) {
