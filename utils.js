...
async function writelog(req, str) {
  ...
  if (req) {
    ...
    str = `[${newDate("{0}-{1}-{2} {3}:{4}")}] - ${username || ''}${account ? `(${account})` : ''}(${req._ip}) - ${str} - ${req._os}\n`
  } else {
...
function handleMusicList(arr) {
  arr.forEach((v, i) => {
    if (i === 0) return;
    if (v.item.length != 0) {
      v.pic = encodeURI(`${mediaurl.fileurl || ''}/music/${v.item[0].artist}-${v.item[0].name}.jpg`);
    } else {
      v.pic = 'img/music.jpg';
    }
  });
