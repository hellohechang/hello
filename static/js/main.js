~async function () {
  ...
  var $document = $(document),
    ...
    $logtop = $(".logtop"),
    $guestbooklisttop = $(".guestbooklisttop"),
  ...
  function userlenght(str) {
    let arr = str.split(''),
      num = 0;
    arr.forEach(v => {
      num += /[\u2E80-\u2FDF\u3040-\u318F\u31A0-\u31BF\u31F0-\u31FF\u3400-\u4DB5\u4E00-\u9FFF\uA960-\uA97F\uAC00-\uD7FF]/.test(v) ? 2 : 0.8;
    });
    if (num > 12) {
      return true;
    } else {
  ...
  _getAjax("/user/getuserinfo").then((result) => {
    if (parseInt(result.code) === 0) {
      ...
      if (dmwidth > 800) {
        if (_userinfo.bg != '') {
          $bgmain.css("background-image", `url(${mediaURL}/bgys/bg/${_userinfo.bg})`);
          imgjz(`${mediaURL}/bgys/bg/${_userinfo.bg}`, () => {
            loadingdh();
        ...
        if (_userinfo.bgxs != '') {
          $bgmain.css("background-image", `url(${mediaURL}/bgys/bgxs/${_userinfo.bgxs})`);
          imgjz(`${mediaURL}/bgys/bgxs/${_userinfo.bgxs}`, () => {
            loadingdh();
  ...
  function handleNavLogo() {
    ...
    $('.sidenav-btn').each((index, item) => {
      ...
      imgjz(url, () => {
        $item.find('.suqlogo').css('background-image', `url(${url})`);
      }, () => {
        $item.find('.suqlogo').css('background-image', `url('https://api.iowen.cn/favicon/${getHost(link)}.png')`);
      });
    });
  }
  //侧边导航缩放
  ...
  $menuid.on("click", ".blockquote", function () {
  ...
  }).on("contextmenu", ".blockquote", function (e) {//操作列表
    e.preventDefault();
    let $this = $(this);
    blockquotemenu(e, {
      id: $this.attr('data-id'),
  ...
  }).on("contextmenu", ".sidenav-btn", function (e) {//操作书签
    e.preventDefault();
    let $this = $(this);
    sidenavbtnmenu(e, {
    ...
    $sidenav.find('.seleckbook').attr('check', che).css('background-color', che === 'y' ? checkColor : 'transparent')
    ...
    let $sidenav = $('.sidenav-btn'),
      $checkArr = $sidenav.filter((_, item) => $(item).find('.seleckbook').attr('check') === 'y');
    ...
    rightMenu.open(e, str, debounce(function (e) {
      ...
      if (_this) {
        ...
        alert(`确认移动到 ${listname}?`, {
          ...
          handled: m => {
            if (m !== 'confirm') return;
            _postAjax("/nav/bmktolist", { pid, arr, toid }).then((result) => {
              if (parseInt(result.code) === 0) {
                rightMenu.close()
                sendCommand({ type: 'updatedata', flag: "bookmark" });
                renderNav()
  ...
  function blockquotemenu(e, obj) {
    ...
    rightMenu.open(e, str, debounce(function (e) {
      ...
      if (_getTarget(e, '.mtcitem')) {
        ...
        rightMenu.open(e, str, debounce(function (e, inp) {
          if (_getTarget(e, '.mtcbtn')) {
            let ni = inp[0].value.trim();
            if (ni === '') {
              _err('请输入列表名');
              return;
            }
            if (ni === obj.name) return;
            _postAjax("/nav/editlist", { id: obj.id, name: ni }).then((result) => {
              if (parseInt(result.code) === 0) {
                rightMenu.close()
                sendCommand({ type: 'updatedata', flag: "bookmark" });
                renderNav()
      ...
      } else if (_getTarget(e, '.mtcitem1')) { // 新增书签
        ...
        rightMenu.open(e, str, debounce(function (e, inp) {
          if (_getTarget(e, '.mtcbtn')) {
            ...
            _postAjax("/nav/addbmk", { id: obj.id, obj: { name: an, link: al, logo: logo } }).then(result => {
              if (parseInt(result.code) === 0) {
                rightMenu.close()
                sendCommand({ type: 'updatedata', flag: "bookmark" });
      ...
      } else if (_getTarget(e, '.mtcitem2')) {//分享列表
        _postAjax('/nav/share', { id: obj.id }).then((result) => {
          if (parseInt(result.code) === 0) {
            rightMenu.close()
            let url = `${sideURL}/page/sharebm/#${result.data.id}`;
            makeCode(url);
          }
        });
      } else if (_getTarget(e, '.mtcitem3')) {//删除列表
  ...
  function sidenavbtnmenu(e, obj) {
    let str = `<div cursor class="mtcitem6">在新标签页打开</div>
                  <div cursor class="mtcitem">二维码</div>
                  <div cursor class="mtcitem1">更换图标</div>
                  ${$footmenu.is(':hidden') ? '<div cursor class="mtcitem2">批量操作</div>' : ''}
                  <div cursor class="mtcitem3">编辑书签</div>
                  <div cursor class="mtcitem4">移动到</div>
                  <div cursor class="mtcitem5">删除</div>`;
    rightMenu.open(e, str, debounce(function (e) {
      if (_getTarget(e, '.mtcitem')) {//二维码
        rightMenu.close()
        makeCode(obj.link);
      } else if (_getTarget(e, '.mtcitem1')) {//上传图标
          ...
          fileReader.onload = (e) => {
            ...
            _upFile(`/home/bmklogo?name=${HASH}`, blob, function (pes) {
              ...
              if (parseInt(result.code) === 0) {
                ...
                _postAjax("/nav/setlogo", { pid: obj.pid, cid: obj.cid, purl }).then((result) => {
                  if (parseInt(result.code) === 0) {
                    sendCommand({ type: 'updatedata', flag: "bookmark" });
                    renderNav();
                    return;
        ...
        rightMenu.open(e, str, debounce(function (e, inp) {
          if (_getTarget(e, '.mtcbtn')) {
            ...
            let requestObj = { pid: obj.pid, cid: obj.cid, name: an, logo: obj.logo, link: al }
        ...
        rightMenu.open(e, str, debounce(function (e) {
          ...
          if (_this) {
            ...
            alert(`确认移动到 ${listname}?`, {
              ...
              handled: m => {
                if (m !== 'confirm') return;
                _postAjax("/nav/bmktolist", { pid: obj.pid, arr: [obj.cid], toid }).then((result) => {
                  if (parseInt(result.code) === 0) {
                    rightMenu.close()
                    sendCommand({ type: 'updatedata', flag: "bookmark" });
                    renderNav()
      ...
      } else if (_getTarget(e, '.mtcitem5')) {// 删除书签
        alert(`确认删除？`, {
          confirm: true,
          handled: m => {
            if (m !== 'confirm') return;
            _postAjax("/nav/delbmk", { pid: obj.pid, arr: [obj.cid] }).then((result) => {
              if (parseInt(result.code) === 0) {
                rightMenu.close()
                sendCommand({ type: 'updatedata', flag: "bookmark" });
                renderNav()
  ...
  function renderhomebook() {
    ...
    $('.folder-item').each((index, item) => {
      ...
      if (!url.includes('favicon.ico') && $item.find('.folder-item-box').attr('x') !== 'add') {
        url = mediaURL + url
      }
      imgjz(url, () => {
        $fib.css({
  ...
  $searchbox.on("click", ".folder-item-box", debounce(function (e) {
    e.stopPropagation();
    let $this = $(this)
    if ($this.attr('x') === 'add') {
      let str = `<div class="mtcinp">
      ...
      rightMenu.open(e, str, debounce(function (e, inp) {
        if (_getTarget(e, '.mtcbtn')) {
          let sqname = inp[0].value.trim(),
            sqlink = inp[1].value.trim();
          if (!isurl(sqlink)) {
            _err('请输入正确的网址');
            return;
          }
  ...
  }, 1000, true)).on("contextmenu", ".folder-item-box", function (e) {
    e.preventDefault();
    let $this = $(this);
    folderitemmenu(e, {
    ...
    $folder.attr('check', che).css('background-color', che === 'y' ? checkColor : 'transparent')
    ...
    let $sidenav = $('.folder-item'),
      $checkArr = $sidenav.filter((_, item) => $(item).attr('data-id') && $(item).find('.celeckhomesq').attr('check') === 'y');
    _success(`选中：${$checkArr.length}`, true)
          ...
          handled: m => {
            ...
            _postAjax("/home/bmktolist", { arr, nid }).then((result) => {
              if (parseInt(result.code) === 0) {
                ...
                sendCommand({ type: 'updatedata', flag: "bookmark" });
                renderHomebook()
                if ($mainid.css("transform") == "none") { } else {
                  renderNav()
                }
                return;
              }
  ...
  function folderitemmenu(e, obj) {
    ...
    rightMenu.open(e, str, debounce(function (e, inp) {
      if (_getTarget(e, '.mtcitem')) {//二维码
        rightMenu.close()
        makeCode(obj.link);
      } else if (_getTarget(e, '.mtcitem1')) { // 自定义图标
        if (_userinfo.account === 'test') {
          _err('测试账号不让更换图标~')
          return;
        }
          ...
          fileReader.onload = (e) => {
            ...
            _upFile(`/home/bmklogo?name=${HASH}`, blob, function (pes) {
              ...
              if (parseInt(result.code) === 0) {
                pro.close()
                let purl = result.data.purl;
                _postAjax("/home/setlogo", { id: obj.id, purl }).then((result) => {
                  if (parseInt(result.code) === 0) {
                    sendCommand({ type: 'updatedata', flag: "bookmark" });
                    renderHomebook()
                    return;
        ...
        rightMenu.open(e, str, debounce(function (e, inp) {
          if (_getTarget(e, '.mtcbtn')) {
            ...
            let requestObj = { id: obj.id, name: b, logo: obj.logo, link: c }
            if (requestObj.logo.includes('favicon.ico')) {
        ...
        rightMenu.open(e, str, debounce(function (e) {
          ...
          if (_this) {
            ...
            alert(`确认移动到 ${listname}?`, {
              ...
              handled: m => {
                if (m !== 'confirm') return;
                _postAjax("/home/bmktolist", { arr: [obj.id], nid }).then((result) => {
                  if (parseInt(result.code) === 0) {
                    rightMenu.close()
                    sendCommand({ type: 'updatedata', flag: "bookmark" });
                    renderHomebook()
  ...
  $ydfy.click(debounce(function () {
    ...
    openIframe(`https://dict.youdao.com/w/${a}`, a);
  }, 500));
  // 搜索提示词
  function tosearch() {
  ...
  $inputTextid.on("input", debounce(function () {
  ...
  $searchlistid.on("click", "li", debounce(function (e) {
    $inputTextid.val("");
    let $this = $(this),
  ...
  $searchbox.on('click', '.manageHistory', debounce(function () {
    ...
    _getAjax('/bg/randombg', { p: _flag }, true).then(result => {
      if (parseInt(result.code) === 0) {
        ...
        imgjz(url, () => {
          ...
          _postAjax("/bg/updatabg", { flag: _flag, bg: result.data.link }, true).then((result) => {
            if (parseInt(result.code) === 0) {
  ...
  function bgitemmenu(e, url) {
    ...
    rightMenu.open(e, str, debounce(function (e, inp) {
      if (_getTarget(e, '.mtcitem')) {
        ...
        _postAjax("/bg/updatabg", { flag: bgflag, bg: url }, true).then((result) => {
          if (parseInt(result.code) === 0) {
            sendCommand({ type: 'updatedata', flag: "userinfo" });
            return;
          }
  ...
  function bgxuanran(y) {
    ...
    _getAjax('/bg/getbg', { flag: _flag, page: bgpage, showpage }).then(result => {
      if (parseInt(result.code) === 0) {
        if ($allbgwrap.is(':hidden')) return
        let {
          total,
  ...
  $allbg.on("contextmenu", ".bgitem", function (e) {
    e.stopPropagation();
    e.preventDefault();
    let url = $(this).attr("x");
    bgitemmenu(e, url);
  }).on("click", ".bgitem", function () {
    let psrc = $(this).children(".bgitemimg").attr("data-src"),
      pp = psrc.replace('bgys', 'bg');
    imgPreview(pp, psrc)
  ...
  }, 500, true)).on('scroll', debounce(function () {
    bglazyImg($allbg, '.bgitem', '.bgitemimg');
  }, 100))
  if (isios()) {
  ...
  //设置
  $setUp.click(debounce(function (e) {
    ...
    rightMenu.open(e, str, debounce(function (e) {
      if (_getTarget(e, '.mtcitem')) {
        ...
        rightMenu.open(e, str, debounce(function (e) {
          if (_getTarget(e, '.mtcitem')) {
            ...
            alert("确认删除账号？", {
              ...
              handled: m => {
                if (m === 'confirm') {
                  _postAjax("/user/delaccount", {}).then((result) => {
                    if (parseInt(result.code) === 0) {
        ...
        let str = `<div cursor class="mtcitem">壁纸库</div>
                <div cursor class="mtcitem1">模糊度</div>
              <div cursor class="mtcitem2">灰度</div>
              <div cursor class="mtcitem3">字体</div>
              <div cursor data-mouse="${mymouse}" class="mtcitem4">鼠标跟随 [${mymouse === "y" ? '开' : '关'}]</div>
              <div cursor data-dian="${dian}" class="mtcitem6">点击爱心 [${dian === "y" ? '开' : '关'}]</div>
              <div cursor data-hua="${hua}" class="mtcitem5">樱花飘落 [${hua === "y" ? '开' : '关'}]</div>
        ...
        rightMenu.open(e, str, debounce(function (e) {
          ...
          if (_getTarget(e, '.mtcitem')) {
            ...
            _progressBar(_filterbg / 100, throttle(function (per) {
            ...
            _progressBar(_pagecolor, throttle(function (per) {
              _pagecolor = per
            ...
            _getAjax('/user/getfont').then(res => {
              if (res.code == 0) {
                ...
                res.data.forEach((item, idx) => {
                  ...
                  str += `<div cursor data-font="${item}" class="mtcitem"><span style="font-size:16px">${(idx + 1 + '').padStart(2, '0')}. </span>${item === 'consolas.ttf' ? '默认字体' : name}</div>`
                })
                rightMenu.open(e, str, debounce(function (e) {
  ...
  function playallmusic() {
    _getAjax('/player/getall').then((result) => {
      if (parseInt(result.code) === 0) {
        ...
        musicarr = randomplay === false ? myShuffle(deepClone(musics)) : deepClone(musics);
        _playinglist = deepClone(musics);
        _postAjax("/player/updateplaying", { data: _playinglist }).then((result) => {
          if (parseInt(result.code) === 0) {
  ...
  function audioPlay() {
    ...
    if (isddplay) {//远程播放
      ...
      $titleid.text(`\xa0\xa0\xa0♪Playing：${musicobj.artist} - ${musicobj.name}`);
      $myAudio[0].play();
      if ($myAudio[0].buffered.length === 0) {//音乐没有缓冲
        musicInitial()
      }
  ...
  function musiclrc() {
    ...
    _getAjax("/player/lrc", {
      ...
      if (parseInt(result.code) === 0) {
        ...
        rod.forEach(item => {
          str += `<div>
          <p style="text-align:${lrcstatu.statu};font-size:${lrcstatu.size + 'px'};line-height:${lrcstatu.size + 6 + 'px'}" class="elrc">${item.p}</p>
          <p style="display: ${showlrcfy && hasfy ? 'block' : 'none'};text-align:${lrcstatu.statu};font-size:${lrcstatu.size - 2 + 'px'};line-height:${lrcstatu.size + 4 + 'px'}" class="lrcfy">${item.fy}</p></div>`
        })
        $lrc.html(str)
        handleLrc(true)
      }
  ...
  let lrcCount = -1; //歌词计数
  $myAudio.on('loadedmetadata', function () {//元数据加载完
    $myAudio[0]._totalTime = (isNaN($myAudio[0].duration) || $myAudio[0].duration === Infinity) ? 0 : $myAudio[0].duration;
    $totaltime.text(tin($myAudio[0]._totalTime));
  }).on('waiting', function () {//缺少数据加载效果
  ...
  }).on('playing', debounce(function () {//准备开始播放
    if ($myAudio[0].paused) return;
    ...
    rod.filter(item => item.t == timm).forEach((item, i) => {
      lrcCount++
      ...
      _setTimeout(() => {
        ...
        let activep = '',
          activep1 = '';
        activep = showlrcfy ? `${rod[actionLrcIndex].p} ${rod[actionLrcIndex].fy}` : rod[actionLrcIndex].p
        if (actionLrcIndex + 1 === rod.length) {
          activep1 = ''
        } else {
          activep1 = showlrcfy ? `${rod[actionLrcIndex + 1].p} ${rod[actionLrcIndex + 1].fy}` : rod[actionLrcIndex + 1].p
        }
        if ($lrcbot.attr('x') === '0') {
  ...
  // 音乐分享
  $musicshare.click(debounce(function (e) {
    e.stopPropagation();
    if (!musicobj) return;
    let obj = {
      name: musicobj.name,
    ...
    _postAjax('/player/musicshare', obj).then(result => {
      if (parseInt(result.code) === 0) {
        let url = `${sideURL}/page/sharemusic/#${result.data.id}`;
        makeCode(url);
      }
    });
  }, 500, true));
  // 歌词设置样式和编辑
  let lrcstatu = _getData('lrcstatu') || { size: dmwidth > 800 ? 14 : 18, statu: 'center' }
  $looklrc.click(debounce(function (e) {
    e.stopPropagation()
    ...
    rightMenu.open(e, str, function (e, inp) {
      if (_getTarget(e, '.mtcitem')) {
        let { size } = lrcstatu;
        size += 2
  ...
  function proTime(pes, y) {
    ...
    let val = (pro1.offsetWidth - dolt.offsetWidth) * pes + dolt.offsetWidth / 2 + 'px';
    let per = pes * 100 + "%";
    if (dolt.offsetWidth) {
  ...
  $yuanchengbfid.click(function (e) {
    if (isddplay) {
      ...
      _success("远程播放已开启");
      $myAudio[0]._rod = []
      $lrc.html(`<div style="width:100%;font-size:18px;position: fixed;text-align: center;top: 45%;left:50%;transform: translateX(-50%);">远程播放中...</div>`);
      audioPause();
  ...
  function musicobjInit(obj) {
    ...
    musicobj.url = encodeURI(`${mediaURL}/music/${musicobj.artist}-${musicobj.name}.mp3`);
    musicobj.murl = encodeURI(`${mediaURL}/music/${musicobj.artist}-${musicobj.name}.mp4`);
  }
  function musicInitial() {
    ...
    $myAudio[0]._rod = []
    $lrc.html(`<div style="width:100%;font-size:18px;position: fixed;text-align: center;top: 45%;left:50%;transform: translateX(-50%);">正在获取歌词...</div>`);
  }
  let playtimer = null
  function musicPlay(obj) {
    ...
    playtimer = setTimeout(() => {
      ...
      if (!isddplay) {//未开启远程
        ...
        _postAjax('/player/updatemusicinfo', {//更新当前播放音乐
          history: 'y',
          lastplay: obj,
          currentTime: $myAudio[0].currentTime,
          duration: $myAudio[0]._totalTime
          ...
          if (result.code == 0) {
            ...
            if (!$mmlistid.is(':hidden')) {
              if (_music && _music.findIndex(item => item.id === $mmlist._flagId) === 0 && $mmmlist.css('transform') === 'none') {
                renderMusicItem()
              }
            }
          }
  ...
  $playingbot.on("click", ".dleplaying", function (e) {
    ...
    gaolianging(false);
    _postAjax(
      "/player/updateplaying", { data: _playinglist }).then((result) => {
        if (parseInt(result.code) === 0) {
          sendCommand({ type: 'updatedata', flag: "playinglist" });
          return;
        }
  ...
  $msearchlistid.on("click", ".limusic", function (e) {
    ...
    musicarr = randomplay === false ? myShuffle(deepClone(musicarrjl)) : deepClone(musicarrjl);
  ...
  probox.addEventListener("touchstart", function (e) {
    ...
    function mmove(e) {
      ...
      pes = (ev.clientX - a - dolt.offsetWidth / 2) / (pro1.offsetWidth - dolt.offsetWidth)
      proTime(pes);
    }
    function mend(e) {
      if (isddplay) {
  ...
  probox.addEventListener('mousedown', function (e) {
    ...
    function mmove(e) {
      e.preventDefault();
      let a = pro1.offsetLeft + mmlist.offsetLeft - $mmlistid.width() / 2;
      pes = (e.clientX - a - dolt.offsetWidth / 2) / (pro1.offsetWidth - dolt.offsetWidth)
      proTime(pes);
    }
    function mup() {
      if (isddplay) {
  ...
  $vobellid.on("click", function (e) {
    _progressBar(defaultvolume, throttle(function (per, type) {
      defaultvolume = per
      vobellm()
      if (type === 'up') {
        if (isddplay) {
  ...
  (function () {
    let fromDom = null;
    $mmlist.on("dragstart", ".gedanlist", function (e) {
  ...
  function rendermusiclist() {
    ...
    str += `${_userinfo.account === 'root' ? '<li cursor class="addslist"><img src="/img/tianjia.png"></li>' : ''}`;
    $mmlist.children('.mmitemwrap').html(str);
  ...
  function rendermusicitem() {
    ...
    str += `<div class="lbxianshi">
       <div class="gdfmtop">
         <img onload="this.style.display='block'" onerror="this.onerror=''; src='/img/music.jpg'" src="${marr.pic}">
       </div>
       <div class="gdlbname">
         <div class="lbming" title="${name}">${name}</div>
         ${_userinfo.account === 'root' && ind > 1 ? `<i cursor class="editlb iconfont icon-bianji"></i>` : ''}
       </div>
    ...
    marr.item.forEach(item => {
      str += `<div title="${item.artist}-${item.name}" draggable="true" m="${item.mv}" xn="${item.name}" xa="${item.artist}" cursor class="songlist">
        <div cursor check="n" class="duoxuan"></div>
        <div class="songlistnum">
          <img class="songlistlogo" data-src=${encodeURI(`${mediaURL}/musicys/${item.artist}-${item.name}.jpg`)}>
          <img class="songlistdq" src="/img/wave.gif">
        </div>
        <div class="songms">
          <span class="divmusicname">${item.name}</span>
          <span class="musicartist"><i class="viptu iconfont icon-vip1"></i>${item.artist}</span>
        </div>
        ${item.mv === 'y' ? ` <div class="mvplay iconfont icon-shipin"></div>` : ''}
  ...
  //打开列表
  $mmlist.on("click", ".gedanlist", throttle(function (e) {
  ...
  }, 1000)).on('scroll', debounce(function () {
    lazyImg($mmlist, '.gedanlist', 'img');
  }, 100)).on("click", '.addslist', function (e) { // 添加歌单
    if (_userinfo.account !== 'root') return;
    let str = `<div class="mtcinp">
  ...
  $mmmlist.on("click", ".editlb", function (e) {
    ...
    rightMenu.open(e, str, debounce(function (e, inp) {
      if (_getTarget(e, '.mtcbtn')) {
        let wz = inp[0].value.trim();
        let nn = inp[1].value.trim();
        if (nn === '') {
          _err('请输入歌单名');
          return;
        }
        ...
        if (isNaN(wz)) return
        _postAjax("/player/editlist", { id, name: nn, index: wz }).then((result) => {
          if (parseInt(result.code) === 0) {
            rightMenu.close()
            sendCommand({ type: 'updatedata', flag: "music" });
            renderMusicList()
  ...
  }).on("click", ".addmusic", async function (e) {// 上传歌曲
    e.stopPropagation();
    ...
    $input.change((e) => {
      ...
      async function fn(num) {
        ...
        if (!/^[^\-]*[^\s\-]\-[^\s\-][^\-]*\.(jpg|mp3|lrc|mp4)$/i.test(name)) {
          pro.fail()
          _err(`${name} 格式错误`);
          num++;
          fn(num);
  ...
  }).on("click", ".bfdq", playList).on('scroll', debounce(function () {
    lazyImg($mmmlist, '.songlist', '.songlistlogo');
  }, 100)).on("click", ".addtop", function (e) {//添加到播放列表
    e.stopPropagation();
    let mobj = {
    ...
    $('.qxbtn').attr('x', '1')
  }).on("click", ".qxbtn", function () {//全选/全不选
    let $this = $(this),
      $duoxuan = $(".duoxuan");
    let num = 0
    if ($this.attr("x") === "1") {
  ...
  }).on("click", ".schxbtn", function (e) {//收藏选中
    e.stopPropagation();
    let $songlist = $('.songlist'),
      $selectarr = $songlist.filter((index, item) => $(item).find('.duoxuan').attr('check') === 'y');
  ...
  }).on("click", ".qxycbtn", function (e) {// 移除全选
    e.stopPropagation();
    let $songlist = $('.songlist'),
      id = $mmlist._flagId,
      $selectarr = $songlist.filter((index, item) => $(item).find('.duoxuan').attr('check') === 'y');
  ...
  }).on("click", ".qxydbtn", function (e) {// 全选移动
    e.stopPropagation();
    if (_userinfo.account !== 'root') return;
    let $songlist = $('.songlist'),
      id = $mmlist._flagId,
    ...
    rightMenu.open(e, str, debounce(function (e, inp) {
      ...
      if (_this) {
        ...
        alert(`确认移动到 ${listname}?`, {
          ...
          handled: m => {
            if (m !== 'confirm') return;
            _postAjax("/player/songtolist", { id, tid, ar: arr }).then((result) => {
              if (parseInt(result.code) === 0) {
                rightMenu.close()
                sendCommand({ type: 'updatedata', flag: "music" });
                renderMusicList()
  ...
  }).on("click", ".alladdtop", function (e) {// 选中添加到播放列表
    e.stopPropagation();
    let $songlist = $('.songlist');
    let $selectarr = $songlist.filter((index, item) => $(item).find('.duoxuan').attr('check') === 'y');
    if ($selectarr.length === 0) return;
    let arr = [];
    $selectarr.each((i, v) => {
      let $v = $(v);
  ...
  }).on("click", ".songlist", function (e) {//播放列表
    e.stopPropagation()
    ...
    if (_getTarget(e, '.songms') || _getTarget(e, '.songlistnum')) {
      ...
      if (ii < 2) {
        ...
        str = `<div cursor class="mtcitem"><i class="iconfont icon-fenxiang_2"></i><span style="margin-left: 10px;">分享歌曲</span></div>
            ...
            ${_userinfo.account === 'root' ? `<div cursor class="mtcitem3"><i class="iconfont icon-icon-test"></i><span style="margin-left: 10px;">移动到</span></div>
            ${sobj.mv === 'y' ? '<div cursor  class="mtcitem4"><i class="iconfont icon-cangpeitubiao_shanchu"></i><span style="margin-left: 10px;">删除 MV</span></div>' : ''}
            <div cursor class="mtcitem1"><i class="iconfont icon-cangpeitubiao_shanchu"></i><span style="margin-left: 10px;">删除</span></div>`: ''}
          `;
      }
      rightMenu.open(e, str, debounce(function (e) {
        if (_getTarget(e, '.mtcitem')) {
          ...
          $(".xzgd").on("click", function (e) {
            ...
            alert(`确认移动到 ${listname}?`, {
              ...
              handled: m => {
                if (m !== 'confirm') return;
                _postAjax("/player/songtolist", { id, tid, ar }).then((result) => {
                  if (parseInt(result.code) === 0) {
                    rightMenu.close()
                    sendCommand({ type: 'updatedata', flag: "music" });
                    renderMusicList()
    ...
    _success(tz);
    rendermusicitem()
  }).on("click", "#mmtop", function () {//列表回到顶部底部和定位到当前播放歌曲
  ...
  function zidonghide(timemax, el, ell, fn, fn2, fel) {
    ...
    $(el).on("mouseup touchstart mousemove", function (e) {
      if (timer !== null) {
        clearTimeout(timer);
        timer = null
      }
  ...
  drag($guestbooklisttop[0], $guestbooklist[0]);
  drag($userinfobox[0], $userinfobox[0]);
  drag($logtop[0], $logdata[0]);
  drag($musichide[0], $musichide[0], "lastmusichide");
  drag($lrcbotwrap[0], $lrcbotwrap[0], "lastwingc");
  drag($musicmv[0], $musicmv[0]);
  drag($(".onoffbox")[0], $mmlistid[0]);
  drag($editlrc.find('.editlrcbtn')[0], $editlrc[0]);
  ...
  $userinfobox.on('click', '.edituname', function (e) {
    if (_userinfo.account === 'test') {
      _err('测试账号不让改昵称~')
      return;
    }
    ...
    rightMenu.open(e, str, debounce(function (e, inp) {
      if (_getTarget(e, '.mtcbtn')) {
        ...
        if (!/^\w+((-\w+)|(\.\w+))*@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/.test(uname)) {
          _err("邮箱格式错误");
          return;
        }
  ...
  }).on('click', '.dailybg', debounce(function () {
  ...
  $userinfologo.click((e) => {
    ...
    rightMenu.open(e, str, debounce(function (e) {
      if (_getTarget(e, '.mtcitem')) {
        ...
        imgPreview(`${mediaURL}/logo/${_userinfo.account}/${_userinfo.account}.png`)
      }
    }, 1000, true))
  });
  function handleUserinfo() {
    let str = `<ul><li>昵称</li><li>${encodeHtml(_userinfo.username)}</li><li cursor class="edituname">修改</li></ul>
    <ul><li>账号</li><li>${_userinfo.account}</li></ul>
    <ul><li>邮箱</li><li class="useremail">${_userinfo.email || '未绑定'}</li><li class="bindEmail" cursor>${_userinfo.email ? '更换' : '绑定'}</li></ul>
    <ul><li>壁纸</li><li>每日自动更换壁纸</li><li class="dailybg" cursor>${_userinfo.dailybg && _userinfo.dailybg === 'y' ? '开' : '关'}</li></ul>`;
    $usermsgBox.html(str);
    $userinfologo.css("background-image", `url(${mediaURL}/logo/${_userinfo.account}/${_userinfo.account}.png?v=${Math.random()})`);
  }
  ...
  function logxuanrandefault() {
    ...
    new Array(50).fill(null).forEach((v, i) => {
      ...
      str += `<p style="pointer-events: none;background-color:rgb(255 255 255 / 20%);height:25px;width:100%;margin:10px 0;"></p>
            ${w % 2 === 0 ? '<p style="pointer-events: none;background-color:rgb(255 255 255 / 20%);height:25px;width:100%;margin:10px 0;"></p>' : ''}
  ...
  function logxuanran(y) {
    ...
    _getAjax("/root/logsearch", { page: logpage, context, showpage }).then((result) => {
      if (parseInt(result.code) === 0) {
  ...
  $logbot.on('click', debounce(function (e) {
    let target = e.target,
      flag = target.getAttribute('data-flag');
    if (target.tagName === 'BUTTON') {
      if (flag === 'pre') {
        ...
        let val = document.querySelector('.logbot #paginationBox input').value.trim();
        val = parseInt(val)
        if (isNaN(val)) return;
        logpage = val;
        logxuanran(true)
  ...
  $uploadlog.click(throttle(function () {
    logpage = 1;
    logxuanran(true)
  }, 500));
  $searchlog.on("input", debounce(function () {
    logpage = 1;
    logxuanran(true)
  }, 1000));
  // 清空日志
    ...
    carr.forEach((v, i) => {
      ...
      if (isrc) {
        if (/(\.JPG|\.PNG|\.GIF|\.JPEG)$/ig.test(isrc)) {
          ...
          str += `<div class="yuykuang " style="float: ${!isright ? 'left' : 'right'};background-color: ${textcolor};">
        ...
        str += `<p class="textkuang" style="float: ${!isright ? 'left' : 'right'};background-color: ${textcolor};">${data}`;
        if (isright) {
          str += `<span class="wjright" style="border-left-color:${textcolor}"><span>`;
        } else {
          str += `<span class="wjleft" style="border-right-color:${textcolor}"><span>`;
  ...
  function tongzhi(name, data) {
    playSound(`${sideURL}/img/notice.mp3`)
    _success(`${name}: ${data}`);
    // try {
  ...
  let msgtimer = null
  $guestbooklistcon.on("click", ".usermsg", function (e) {//点击用户头像私聊
    e.stopPropagation();
    ...
    if (a === _userinfo.account) {
      imgPreview(`${mediaURL}/logo/${a}/${a}.png`)
      return
    };
    let str = `
  ...
  }).on("click", ".itemconwrap", function (e) {//下载文件
  ...
  }).on("contextmenu", ".guestbookitemcon", function (e) {//操作消息
    e.preventDefault();
    let $this = $(this),
  ...
  }).on("click", ".yuykuang", function (e) {//播放语音
    ...
    if ($guestbooklistconwrap.outerHeight() < $guestbooklistcon.outerHeight() || nomore.length > 0) return;
    if (this.scrollTop <= 30) {
      let fristel = $guestbooklistcon.find('.guestbookitemwrap').eq(0);
      let flag = fristel.attr('data-id');
      let val = $searchMsg.val().trim()
      _getAjax('/chat/getmsg', { flag, acc: chatobj.account, stu: 1, val }).then(result => {
        if (parseInt(result.code) === 0) {
          if ($guestbooklist.is(':hidden')) return;
          let str = sxliuyan(result.data);
          if (str === '') {
  ...
  function backmsg(e, tt, y, z, f, n, s) {
    let obj = deepClone(chatobj),
      str = `${y === 'null' ? '<div cursor class="mtcitem1">复制</div><div cursor class="mtcitem2">编辑</div>' : '<div cursor class="mtcitem3">下载</div>'}
            ${f === _userinfo.account ? '<div cursor class="mtcitem">删除</div>' : ''}`;
    rightMenu.open(e, str, debounce(function (e) {
      if (_getTarget(e, '.mtcitem')) {
  ...
  function openyuy(a, _this) {
    ...
    $yuyplay[0].src = a;
    $yuyplay[0].play();
    $(_this).children('i').css("animation", "fontcolor .5s infinite linear alternate");
  }
  $yuyplay.on("ended", function () {
    $yuyplay.playflag = '';
    $('.yuykuang').children('i').css('animation', 'none');
  }).on('error', function () {
    _err('语音已过期~');
  ...
  function getNewMsg(y) {
    let pchatId = deepClone(chatobj)
    let $guestbookitemwrap = $guestbooklistcon.find('.guestbookitemwrap').last();
    let flag = $guestbookitemwrap.attr('data-id');
    $searchMsg.val('')
    _getAjax('/chat/getmsg', { stu: 2, acc: pchatId.account, flag }).then(result => {
      if (parseInt(result.code) === 0) {
        if ($guestbooklist.is(':hidden')) return;
        let str = sxliuyan(result.data);
  ...
  $searchMsg.on('input', debounce(function () {
    let val = $searchMsg.val().trim()
    let pchatId = deepClone(chatobj)
    _getAjax('/chat/getmsg', { stu: 0, acc: pchatId.account, val }).then(result => {
      if (parseInt(result.code) === 0) {
        if ($guestbooklist.is(':hidden')) return;
        let str = sxliuyan(result.data);
        $guestbooklistconwrap.html(str);
          ...
          $.ajax({
            ...
            success: (data) => {
              ...
              let obj = {
                ...
                data: "语音",
                size: (blob.size / 1024 / 2).toFixed(2) + "s",
                _from: _userinfo.account,
                name: _userinfo.username,
              };
              sendmasg(obj, pchatId)
              playSound(`${sideURL}/img/voice.mp3`)
            },
            error: () => {
              pro.fail('发送失败')
              _err("发送失败~");
  ...
  ~function () {
    let x = null,
      y = null;
    $guestgo.on("touchstart", function (e) {
      e.preventDefault()
  ...
  $chengyuan.click(debounce(function (e) {
    ...
    $chengyuanwrap.slideDown(_speed, () => {
      ...
      _getAjax("/chat/getmember").then((result) => {
        if (parseInt(result.code) === 0) {
          if ($guestbooklist.is(':hidden') || $chengyuanwrap.is(':hidden')) return;
          result.data.sort((a, b) => b.time - a.time);
          result.data.sort((a, b) => b.online.localeCompare(a.online, 'en'));
          let str = "",
            rnum = Math.random();
          result.data.forEach((v) => {
            ...
            str += `<ul cursor xx="${username}" xxx="${account}" class="cyitem">
              ...
              <li class="cylogo" style="${online === 'y' ? '' : 'filter: grayscale(1);'}background-image:url('${mediaURL}${logo}?v=${rnum}');"></li>
              <li class="cyname">${username}</li>
              <li style="font-size:14px;line-height:40px;flex:none;color:${online === 'y' ? 'green' : '#aaa'};">${online === 'y' ? '在线' : '离线'}</li>
              </ul>`;
          });
          $chengyuanwrap.html(str);
          $msgtx.fadeOut(_speed);
  ...
  ~function () {
    ...
    let id = nanoid(),
      fg = 0;
    function realtime() {
      _getAjax('/user/realtime', { flag: fg, id }, true, true).then(resu => {
        if (resu.code == 0) {
          ...
          if (type === 'chat') {
            ...
            if (flag === 'addmsg') {//新消息处理
              // 消息通知
              _getAjax('/chat/notice', { acc: to === 'chang' ? 'chang' : from.account }).then(result => {
                if (parseInt(result.code) === 0) {
                  ...
                  if ($guestbooklist.is(":hidden")) {//聊天框是隐藏
                    ...
                    if ((pid.account === from.account && to !== 'chang') || (pid.account === 'chang' && to === 'chang') || (from.account === _userinfo.account && pid.account === to)) {//新消息是当前聊天框
                      let acc = to === 'chang' ? 'chang' : (from.account === _userinfo.account && pid.account === to ? to : from.account)
                      let flag = $('.guestbookitemwrap').last().attr('data-id');
                      let val = $searchMsg.val().trim()
                      _getAjax('/chat/getmsg', { stu: 2, acc, flag, val }).then(result => {
                        if (parseInt(result.code) === 0) {
                          if ($guestbooklist.is(':hidden')) return;
                          let data = result.data;
          ...
          } else if (type === 'updatedata') {//数据同步更新
            if (flag === "music") {
              renderMusicList()
              ...
              _getAjax("/player/getplaying").then((result) => {
                if (parseInt(result.code) === 0) {
                  _playinglist = result.data;
                  musicarr = randomplay === false ? myShuffle(deepClone(_playinglist)) : deepClone(_playinglist);
                  return;
                }
              }, true);
              ...
              if (!$mmlistid.is(':hidden')) {
                if ($myAudio[0].paused) {
                  _getAjax('/player/getmusicinfo', {}, true).then(result => {
                    if (parseInt(result.code) === 0) {
                      let _musicinfo = result.data;
                      let { currentTime = 0, duration = 0, lastplay } = _musicinfo;
        ...
        realtime()
      }).catch(err => {//网络错误10秒重新请求一次
        _setTimeout(realtime, 10000)
      })
    }
    realtime()
    window.sendCommand = function (data) {//发送指令
      _postAjax('/user/realtime', { data, id }, true)
    }
  }();
  ~function () {
    ...
    function openIframe(url, name) {
      ...
      $box.on('click', debounce(function (e) {
        setZindex($box)
        if (_getTarget(e, '.iframeClose') || _getTarget(e, '.iframeClosexs')) {
          $box.css('height', 0)
          _setTimeout(() => {
