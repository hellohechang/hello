...
route.get('/share', async (req, res) => {
  try {
    let { id } = req.query;
    let arr = await queryData('share', '*', `WHERE id=? AND type=?`, [id, 'bookmk']);
    if (arr.length === 0) {
      _err(res, '分享已被取消~')
      return
    }
...
route.get('/getlist', async (req, res) => {
  try {
    ...
    let { id } = req.query;
    let it = bookSort(await queryData('bookmk', 'id,name,link,logo,num', `WHERE listid=? AND state=? AND account=?`, [id, '0', account]));
    let side = bookSort(await queryData('booklist', 'id,name,num', `WHERE state=? AND account=?`, ['0', account]));
    side = side.map(item => {
      if (item.id === id) {
        item.item = it
      }
...
route.post('/listmove', async (req, res) => {
  try {
    ...
    let side = bookSort(await queryData('booklist', 'id,num', `WHERE account=? AND state=?`, [account, '0']));
...
route.post('/bmkmove', async (req, res) => {
  try {
    ...
    let it = bookSort(await queryData('bookmk', 'id,num', `WHERE listid=? AND state=? AND account=?`, [pid, '0', account]));
...
route.post('/addlist', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let { name } = req.body;
    let total = (await queryData('booklist', "MAX(num)", `WHERE account=? AND state=?`, [account, "0"]))[0]['MAX(num)'] || 0;
    let id = nanoid();
...
route.post('/dellist', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let { id } = req.body;
    await updateData('booklist', { state: '1' }, `WHERE account=? AND id=? AND state=?`, [account, id, '0'])
    await updateData('bookmk', { state: '1' }, `WHERE account=? AND listid=? AND state=?`, [account, id, '0'])
...
route.post('/setlogo', async (req, res) => {
  try {
    ...
    await updateData('bookmk', { logo: purl }, `WHERE account=? AND id=? AND state=? AND listid=?`, [account, cid, '0', pid])
...
route.post('/editlist', async (req, res) => {
  try {
    ...
    await updateData('booklist', { name }, `WHERE account=? AND state=? AND id=?`, [account, '0', id])
...
route.post('/addbmk', async (req, res) => {
  try {
    let account = req._userInfo.account;
    let { id, obj } = req.body;
    let total = (await queryData('bookmk', "MAX(num)", `WHERE listid=? AND account=? AND state=?`, [id, account, '0']))[0]['MAX(num)'] || 0
...
route.post('/editbmk', async (req, res) => {
  try {
    ...
    await updateData('bookmk', { name, logo, link }, `WHERE account=? AND state=? AND id=? AND listid=?`, [account, '0', cid, pid])
...
route.post('/bmktolist', async (req, res) => {
  try {
    ...
    let total = (await queryData('bookmk', "MAX(num)", `WHERE listid=? AND account=? AND state=?`, [toid, account, '0']))[0]['MAX(num)'] || 0
    let ob = [
      {
        key: 'num',
        where: 'id',
    ...
    await batchUpdateData('bookmk', ob, `account="${account}" AND state="0" AND listid="${pid}"`)
    _success(res)
  } catch (error) {
    await writelog(req, `[${req._pathUrl}] ${error}`)
    _err(res)
...
route.post('/delbmk', async (req, res) => {
  try {
    ...
    await updateData('bookmk', { state: '1' }, `WHERE id IN (${new Array(arr.length).fill('?').join(',')}) AND state=? AND listid=? AND account=?`, [...arr, '0', pid, account])
...
route.post('/share', async (req, res) => {
  try {
    ...
    let r = nanoid(),
      { id } = req.body;
    let arr = bookSort(await queryData('bookmk', "*", `WHERE listid=? AND state=? AND account=?`, [id, '0', account]))
    if (arr.length === 0) {
      _err(res, '当前列表为空~')
      return
    }
